...
class CAAdmobRewardedVideo : AdNetwork
{
    ...
    public override void initialize(string gameObjectName, string uniqueDeviceID)
    {
        try
        {
            admob.initRewardedBasedVideo();
            admob.RequestRewardBasedVideo(appKey); //requesting ad to show for the first time
        }
    ...
    {
        bool result = admob.ShowRewardBasedVideo();
        admob.RequestRewardBasedVideo(appKey); //request a new ad for next time
        return result;
    }
